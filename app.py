...
- issues (課題: 特定された課題)
- specificObstacles (具体的な阻害要因: 音声から的確に阻害要因を抽出して記述)
- plannerActions (プランナー等が行ったこと及び企業からの反応: 【重要】必ず「社労士（プランナー）が行ったこと」と「企業の反応」を見出し等で明確に分け、それぞれを『箇条書き（・）』でプロフェッショナルな報告書文体で出力してください)
